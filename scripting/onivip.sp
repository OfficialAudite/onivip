...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_vip", RCC_vip, "VIP commands", ADMFLAG_RESERVATION);
	RegConsoleCmd("sm_gift", RCC_vip, "Gift VIP to a player", ADMFLAG_RESERVATION);
...
public Action RCC_giftvip(int client, int args){
	if(player[client].rank >= 2){
		// gift vip to other player for 5 days. Only once for 2 players per month
	}else{
